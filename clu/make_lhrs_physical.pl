...
open OUT, '>:utf8', "lhrs_physical.mrc" or die "Cannot open output file: $!\n";
...
while (my $record = UCLA_Batch::safenext($batch)) {
  ...
  # Add 852 $a CLU
  $fld = $record->field('852');
  $fld->add_subfields('a' => 'CLU');

  # Make various changes for certain SRLF locations
  if ($loc eq 'sr' || $loc eq 'srbuo' || $loc =~ /^srucl/) {

    # Change LDR/17 = 2 for certain locs
    my $leader = $record->leader();
    substr($leader, 17, 1) = '2';
    $record->leader($leader); 

    # Delete all 85x and 86x field; we'll add an 852 later
    @flds = $record->field('85.');
    $record->delete_fields(@flds);
    @flds = $record->field('86.');
    $record->delete_fields(@flds);

    # Replace 852 field
    my $sr_852 = MARC::Field->new('852', '', '', a=>'CLU', b=>$loc);
    $record->insert_fields_ordered($sr_852);

    # Remove all 5xx fields
    @flds = $record->field('5..');
    $record->delete_fields(@flds);
...
sub get_policy {
  my $loc = shift;
  my $policy = "uu"; # default: lending and copying policy unknown
  for ($loc) {
    # Auxiliary libraries generally get bb (will not lend, will not copy)
    if    (/^aa/) {$policy = "bb";}
    elsif (/^ai/) {$policy = "bb";}
    elsif (/^ca/) {$policy = "bb";}
    elsif (/^ck/) {$policy = "bb";}
    ...
    # But ERR gets bu (will not lend, copy policy unknown)
    elsif (/^er/) {$policy = "bu";}
  }
  return $policy;
}
...
# Retreive max val for given tag
sub get_max_f85x {
  my ($tag, @arr) = @_;
  my $val = 1;
  if ($tag eq '853') {
