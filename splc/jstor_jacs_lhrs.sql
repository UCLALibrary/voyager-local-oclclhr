...
with srucl2 as (
  ...
  where l.location_code = 'srucl2'
)
, updates as (
  -- srucl2 holdings updated since last_date
  select mfhd_id
  from ucladb.mfhd_history
  where mfhd_id in (select mfhd_id from srucl2)
  and action_date >= to_date('&last_date', 'YYYYMMDD')
  and operator_id != 'nomelvyl'
  union
  -- srucl2 holdings, if their *bibs* were updated since last_date
  select s.mfhd_id
  from srucl2 s
